...
@Component({
  selector: 'sdrc-signal-table',
  templateUrl: './table.component.html',
  styleUrls: ['./table.component.scss']
})
export class SignalTableComponent implements OnInit, OnChanges {
  isDesc: any;
  direction: number;
  fixTableConfig: any = {};
  ...
  ngOnChanges() {
    if (this.rowData.length && !this.isPaginate)
    ...
    setTimeout(()=>{
      // if($("#"+this.id).parent()[0].scrollWidth == $("#"+this.id).parent()[0].clientWidth){
        ...
        $("#header-fixed" + this.id + " .th").each(function(index) {
          $(this).width($($("#"+tempThis.id + " th")[index]).outerWidth()-1)
        })

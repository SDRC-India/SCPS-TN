...
export class EditUserDetailsComponent implements OnInit {  
  ...
  ngOnInit() {   
    ...
    if(!this.userManagementService.editUserDetails){
      ...
      this.selectedDesignationId = this.userManagementService.editUserDetails.designationId;
    }
  ...
  getFacilities(){
    ...
    this.userManagementService.getFacilityByRoleAndDistrict(this.selectedDesignationId,this.selectedDistrictId).subscribe(res=>{
      let data = <any[]>res
      data.forEach(el => {
        if(el.name != undefined && el.name != null && el.name.trim().length ==0){
          el.name = el.nameAndAddress
        }
      });
      this.facilities = res;
  ...
  updateUserDetails(roleId:any){ 
    ...
    let areaId =""
     let userDetails = {  
      ...
      "email":this.userManagementService.editUserDetails.emailId,
